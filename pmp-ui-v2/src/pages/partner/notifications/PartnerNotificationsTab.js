    ...
    return (
        <div className={`flex text-xs bg-[#FCFCFC] font-bold ${isLoginLanguageRTL && 'space-x-reverse'} gap-x-4 sm:gap-x-16 items-start rounded-lg px-8`}>
            <div id='partner_cert_notifications_tab' className={`flex-col justify-center text-center w-full sm:w-auto`}>
                <button onClick={changeToPartnerCert} className={`${activeTab === "PARTNER_CERT_EXPIRY" ? "text-[#1447b2]" : "text-[#031640]"} py-4 cursor-pointer text-base`}>
                    <h6> {t('partnerNotificationsTab.partnerCertificate')} </h6>
                </button>

                <div className={`h-1 w-full ${activeTab === "PARTNER_CERT_EXPIRY" ? "bg-tory-blue" : "bg-transparent"}  rounded-t-md`}></div>
            </div>
            { partnerType === "FTM_PROVIDER" && (
                <div id='ftm_cert_notifications_tab' className={`flex-col justify-center text-center w-full sm:w-auto`}>
                    <button onClick={changeToFtmChipCert} className={`${activeTab === "FTM_CHIP_CERT_EXPIRY" ? "text-[#1447b2]" : "text-[#031640]"} py-4 cursor-pointer text-base`}>
                        <h6> {t('partnerNotificationsTab.ftmCertificate')} </h6>
                    </button>

                    <div className={`h-1 w-full ${activeTab === "FTM_CHIP_CERT_EXPIRY" ? "bg-tory-blue" : "bg-transparent"}  rounded-t-md`}></div>
                </div>
            )}
            { partnerType === "AUTH_PARTNER" && (
                <div id='api_key_notifications_tab' className={`flex-col justify-center text-center w-full sm:w-auto`}>
                    <button onClick={changeToApiKeyTab} className={`${activeTab === "API_KEY_EXPIRY" ? "text-[#1447b2]" : "text-[#031640]"} py-4 cursor-pointer text-base`}>
                        <h6> {t('partnerNotificationsTab.apikey')} </h6>
                    </button>

                    <div className={`h-1 w-full ${activeTab === "API_KEY_EXPIRY" ? "bg-tory-blue" : "bg-transparent"}  rounded-t-md`}></div>
                </div>
            )}
            { partnerType === "DEVICE_PROVIDER" && (
                <div id='sbi_notifications_tab' className={`flex-col justify-center text-center w-full sm:w-auto`}>
                    <button onClick={changeToSbiTab} className={`${activeTab === "SBI_EXPIRY" ? "text-[#1447b2]" : "text-[#031640]"} py-4 cursor-pointer text-base`}>
                        <h6> {t('partnerNotificationsTab.sbi')} </h6>
                    </button>

                    <div className={`h-1 w-full ${activeTab === "SBI_EXPIRY" ? "bg-tory-blue" : "bg-transparent"}  rounded-t-md`}></div>
                </div>
            )}
        </div>
    );

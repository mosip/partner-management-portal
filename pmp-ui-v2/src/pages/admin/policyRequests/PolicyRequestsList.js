  ...
  const ensureEligibilityLoaded = useCallback(async (policyRequest) => {
    ...
    try {
      const [bioResp, credResp] = await Promise.all([
        ...
        usePartnerEndpoint
          ? HttpService.get(getPartnerManagerUrl(`/partners/${policyRequest.partnerId}/policies/${policyRequest.policyId}/credentialTypes`, process.env.NODE_ENV))
  ...
  return (
    <div className={`mt-2 w-[100%] ${isLoginLanguageRTL ? "mr-28 ml-5" : "ml-28 mr-5"} overflow-x-scroll font-inter`}>
      ...
      {dataLoaded && (
        <>
          {errorMsg && (
            <ErrorMessage id='policy_requests_list_error_msg' errorCode={errorCode} errorMessage={errorMsg} clickOnCancel={cancelErrorMsg} />
          )}

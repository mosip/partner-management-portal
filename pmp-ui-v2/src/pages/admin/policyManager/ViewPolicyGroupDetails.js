...
function ViewPolicyGroupDetails() {
    ...
    useEffect(() => {
        const selectedPolicyGroup = sessionStorage.getItem('selectedPolicyGroupAttributes');
                    ...
                    <div className={`bg-[#FCFCFC] w-full mt-3 rounded-lg shadow-lg items-center`}>
                        <div className="flex items-center justify-center p-24">
                            <div className="flex flex-col justify-center items-center">
                                ...
                                <p id='view_policy_group_unexpected_error' className="text-sm font-semibold text-[#6F6E6E] py-4">{t('devicesList.unexpectedError')}</p>
                                <button onClick={moveToPolicyGroupList} type="button" id='view_policy_group_go_back_btn'
                    ...
                    <div className="bg-snow-white h-fit mt-1 rounded-t-xl shadow-lg font-inter">
                        <div className="flex justify-between px-7 pt-3 border-b max-[450px]:flex-col">
                            <div className="flex-col">
                                <p id='view_policy_group_sub_title_id' className="text-lg text-dark-blue mb-2 break-all">
                                    {t('policyGroupList.policyGroupId')}: <span className="font-semibold">{policyGroupDetails.id}</span>
                                </p>
                                <div className="flex items-center justify-start mb-2 max-[400px]:flex-col max-[400px]:items-start">
                                    <div id='view_policy_group_status' className={`${bgOfStatus(policyGroupDetails.isActive ? 'ACTIVE' : 'INACTIVE', t)} flex w-fit py-1 px-5 text-sm rounded-md my-2 font-semibold`}>
                                        {getStatusCode(policyGroupDetails.isActive ? 'active' : 'inactive', t)}
                                    </div>
                                    <div id='view_policy_group_created_on' className={`font-semibold ${isLoginLanguageRTL ? "mr-[1.4rem]" : "ml-[0.75rem]"} text-sm text-dark-blue`}>
                                        {t("viewDeviceDetails.createdOn") + ' ' +
                                            formatDate(policyGroupDetails.crDtimes, "date")
                                        }
                                    </div>
                                    <div className="mx-2 text-gray-300">|</div>
                                    <div id='view_policy_group_created_date_time' className="font-semibold text-sm text-dark-blue">
                        ...
                        <div className={`${isLoginLanguageRTL ? "pr-8 ml-8" : "pl-8 mr-8"} pt-3 mb-3`}>
                            <div className="flex flex-wrap py-3 max-[450px]:flex-col">
                                <div className="w-[50%] max-[600px]:w-[100%] mb-3">
                                    <p id='view_policy_group_name_label' className="font-[600] text-suva-gray text-sm">
                                        {t("policyGroupList.policyGroupName")}
                                    </p>
                                    <p id='view_policy_group_name_context' className="font-[600] text-vulcan text-base break-all">

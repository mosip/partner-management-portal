            ...
            <FocusTrap focusTrapOptions={{ initialFocus: false, allowOutsideClick: true }}>
                <div className='flex-col bg-white w-[45rem] max-h-[32rem] mx-[4%] mt-[5%] rounded-md relative'>
                    <div className='flex py-2 px-5 justify-between items-center'>
                        <h2 id='view_policy_popup_title' className='font-semibold text-sm'>{title}</h2>
                        <div className='flex gap-x-9 items-center'>
                            <button id='view_policy_popup_download_btn' onClick={() => downloadJsonFile(previewData)} className='text-xs px-[1.3rem] py-[0.8%] border-2 font-semibold rounded-md h-9 w-fix text-tory-blue bg-white border-blue-800 cursor-pointer'>
                                {t('commons.download')}
                            </button>
                            <button id='view_policy_popup_close_btn' onClick={closePopUp} className='bg-gray-100 text-center h-6 w-5 text-sm font-semibold rounded-[9rem] cursor-pointer'>
                                <p>X</p>
                            </button>
                        </div>
                    </div>

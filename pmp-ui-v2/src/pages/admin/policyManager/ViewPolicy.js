                            ...
                            <div className={`bg-[#FCFCFC] w-full mt-3 rounded-lg shadow-lg items-center`}>
                                <div className="flex items-center justify-center p-24">
                                    <div className="flex flex-col justify-center items-center">
                                        ...
                                        <p id='view_policy_unexpected_error_label' className="text-base font-semibold text-[#6F6E6E] pt-4">{t('commons.unexpectedError')}</p>
                                        <p id='view_policy_unexpected_error_msg' className="text-sm font-semibold text-[#6F6E6E] pt-1 pb-4">{getErrorMessage(errorCode, t, errorMsg)}</p>
                                        <button onClick={() => navigate(viewPolicyPageHeaders.backLink)} id='view_policy_go_back_btn'
                            ...
                            <div className="bg-snow-white h-fit mt-1 rounded-t-xl shadow-lg font-inter">
                                <div className="flex justify-between px-7 pt-3 border-b max-[450px]:flex-col">
                                    <div className="flex-col">
                                        <p id='view_policy_sub_title_id' className="text-lg text-dark-blue mb-2 break-all">
                                            {t('policiesList.policyId')}: <span className="font-semibold">{viewDetails.policyId}</span>
                                        </p>
                                        <div className="flex items-center justify-start mb-2 max-[400px]:flex-col max-[400px]:items-start">
                                            <div id='view_policy_status' className={`${bgOfStatus(getPolicyStatus(viewDetails), t)} flex w-fit py-1 px-5 text-sm rounded-md my-2 font-semibold`}>
                                                {getStatusCode(getPolicyStatus(viewDetails), t)}
                                            </div>
                                            <div id='view-policy_created_on' className={`font-semibold ${isLoginLanguageRTL ? "mr-[1.4rem]" : "ml-[0.75rem]"} text-sm text-dark-blue`}>
                                                {t("viewDeviceDetails.createdOn") + ' ' +
                                                    formatDate(viewDetails.cr_dtimes, "date")
                                                }
                                            </div>
                                            <div className="mx-2 text-gray-300">|</div>
                                            <div id='view_policy_created_date_time' className="font-semibold text-sm text-dark-blue">
                                ...
                                <div className={`${isLoginLanguageRTL ? "pr-4 ml-8" : "pl-4 mr-8"} pt-3 mb-2`}>
                                    ...
                                    <div className={`${isLoginLanguageRTL ? "pr-4" : "pl-4"} pt-3 mb-[3.5rem] w-fit`}>
                                        <p id='view_policy_data_label' className="font-[600] mb-1 text-suva-gray text-sm">
                                        ...
                                        <div className='flex flex-wrap justify-between px-3 items-center h-[5.5rem] border-2 border-[#fedff] rounded-md bg-[#f4f6fb] gap-x-8'>
                                            <div className='flex items-center'>
                                                ...
                                                <p id='view_policy_data' className='font-semibold text-sm mx-2'>{t('viewAuthPoliciesList.policyData')}</p>
                                            </div>
                                            <div role='button' onClick={showUploadedJsonData} className='flex justify-between px-2 py-1.5 w-[6rem] bg-white border-2 border-blue-800 rounded-md hover:cursor-pointer' tabIndex="0" onKeyDown={(e) => onPressEnterKey(e, showUploadedJsonData)}>
                                                <p id='view_policy_data_preview' className='text-xs font-semibold text-blue-800'>{t('viewAuthPoliciesList.preview')}</p>

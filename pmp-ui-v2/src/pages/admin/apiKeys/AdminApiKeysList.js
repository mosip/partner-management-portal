                                                                                ...
                                                                                <div ref={setSubmenuRef(submenuRef, index)}>
                                                                                    ...
                                                                                    {actionId === index && (
                                                                                        <div className={`absolute w-[7%] z-50 bg-white text-xs font-semibold rounded-lg shadow-md border min-w-fit ${isLoginLanguageRTL ? "left-10 text-right" : "right-11 text-left"}`}>
                                                                                            ...
                                                                                            <div role="button" className={`flex justify-between ${disableViewAndDeactivate ? 'cursor-default text-[#A5A5A5]' : apiKey.status === 'activated' ? 'hover:bg-gray-100 cursor-pointer text-[#3E3E3E]' : 'cursor-default text-[#A5A5A5]'}`} onClick={disableViewAndDeactivate ? undefined : () => deactivateApiKey(apiKey, index)} tabIndex={disableViewAndDeactivate ? -1 : 0} onKeyDown={disableViewAndDeactivate ? undefined : (e) => onPressEnterKey(e, () => deactivateApiKey(apiKey, index))}>                            <p id="api_key_list_deactivate_btn" className={`py-1.5 px-4 ${isLoginLanguageRTL ? "pl-10" : "pr-10"}`}>{t("partnerList.deActivate")}</p>

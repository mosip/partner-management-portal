...
function ViewMispLicenseKey() {
    ...
    const [maskedLicenseKey, setMaskedLicenseKey] = useState(true);
    ...
    useEffect(() => {
        ...
        const licenseKey = mispKeyData.mispLicenseKey;
        const visiblePart = licenseKey.slice(-10);
        setMaskedLicenseKey(visiblePart);
    }, []);
            ...
            {!unexpectedError && (
                <div className="bg-snow-white h-fit mt-1 rounded-t-xl shadow-lg font-inter">
                    <div className="flex justify-between px-7 pt-3 border-b max-[450px]:flex-col">
                        ...
                        <button id="view_misp_license_key_details_copy_id" className={`${mispLicenseKeyDetails.status === "activated" ? 'bg-[#F0F5FF] border-[#BED3FF] cursor-pointer hover:shadow-md' : 'bg-gray-200 border-gray-400 cursor-default'}  border h-[4%] w-[14%] max-[450px]:w-[40%] max-[800px]:w-[25%] ${isLoginLanguageRTL ? "pl-[1.5%]" : "pr-[1%]"} py-[0.5%] rounded-md text-right`}>
                            ...
                            <p id="view_misp_license_key_details_id" className={`text-base font-bold ${mispLicenseKeyDetails.status === "activated" ? "text-tory-blue"  : "text-gray-400" }`}>
                                ...{maskedLicenseKey}
                            </p>
                        </button>
                    </div>

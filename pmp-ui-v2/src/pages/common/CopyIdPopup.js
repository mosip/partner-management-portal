...
function CopyIdPopUp({ closePopUp, title, subtitle, id, header, alertMsg, styleSet, removeCopyBtn }) {
                ...
                <div className={`bg-white md:w-[378px] w-[50%] mx-auto rounded-lg shadow-lg h-fit`}>
                    <header className={`flex justify-between p-[5%]`}>
                        <div className={`flex-col w-[19rem] break-all md:break-normal`}>
                            <h1 id='copy_id_popup_tile' className={`font-bold text-base text-[#333333] break-words`}>{title}</h1>
                            <p id='copy_id_popup_sub_title' className={`text-xs font-bold text-[#717171] ${isLoginLanguageRTL ? "text-right" : "text-left"}`}># {subtitle}</p>
                        </div>
                    ...
                    <hr />
                    <div className={`px-5 py-6 flex-col text-center justify-center items-center`}>
                        {alertMsg && (

...
function OidcClientDetailsPage({ variant }) {
    ...
    const sessionStorageKey = isAdminView ? 'selectedOidcClientAttributes' : 'selectedClientData';
    const backLink = isAdminView
        ? '/partnermanagement/admin/authentication-services/oidc-clients-list'
        : '/partnermanagement/authentication-services/oidc-clients-list';
    const subTitleKey = isAdminView
        ? 'oidcClientsList.listOfOidcClients'
        : 'authenticationServices.authenticationServices';
    const errorWhileGettingKey = isAdminView
        ? 'viewAdminOidcClientDetails.errorWhileGettingOidcClientDetails'
        : 'viewOidcClientDetails.errorWhileGettingOidcClientDetails';
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const response = await HttpService.get(
                    getPartnerManagerUrl(`/oidc-clients/${clientId}`, process.env.NODE_ENV)
                );
                if (response) {
                    const responseData = response.data;
                    if (responseData && responseData.response) {
                        const resData = responseData.response;
                        setOidcClientDetails({
                            ...resData,
                    ...
                    setErrorMsg(t(errorWhileGettingKey));
                }
                setDataLoaded(true);
            } catch (err) {
        ...
        fetchData();
    }, [t, sessionStorageKey, isAdminView, errorWhileGettingKey]);
        ...
        <div className={`w-full p-4 bg-anti-flash-white h-full font-inter break-words max-[450px]:text-sm mb-[2%] ${isLoginLanguageRTL ? "mr-24 ml-1" : "ml-24 mr-1"} overflow-x-scroll`}>
            ...
            {dataLoaded && (
                <>
                    {errorMsg && (
                        <ErrorMessage
                            id={isAdminView ? 'view_admin_oidc_client_details_error_msg' : 'view_oidc_client_details_error_msg'}
                            errorCode={errorCode}
                    ...
                    <div className={`flex-col mt-5 bg-anti-flash-white h-full font-inter break-words max-[450px]:text-sm mb-[2%]`}>
                        <div className="flex justify-between mb-3">
                            <Title
                                title='viewOidcClientDetails.viewOidcClientDetails'
                            ...
                            <div className={`bg-[#FCFCFC] w-full mt-3 rounded-lg shadow-lg items-center`}>
                                <div className="flex items-center justify-center p-24">
                                    <div className="flex flex-col justify-center items-center">
                                        ...
                                        <p
                                            id={isAdminView ? 'view_admin_oidc_clients_unexpected_error' : 'view_oidc_clients_unexpected_error'}
                                            className="text-base font-semibold text-[#6F6E6E] pt-4"
                                        >
                                            {t('commons.unexpectedError')}
                                        </p>
                                        <p
                                            id={isAdminView ? 'view_admin_oidc_clients_unexpected_error_msg' : 'view_oidc_clients_unexpected_error_msg'}
                                            className="text-sm font-semibold text-[#6F6E6E] pt-1 pb-4"
                                        >
                                            {getErrorMessage(errorCode, t, errorMsg)}
                                        </p>
                                        <button
                                            onClick={moveToOidcClientsList}
                                            type="button"
                                            id={isAdminView ? 'view_admin_oidc_clients_go_back_btn' : 'view_oidc_clients_go_back_btn'}
                        ...
                        {!unexpectedError && (
                            <>
                                                ...
                                                <div className="mt-4 border-b border-gray-200 pb-3">
                                                    <div className="flex justify-between items-start max-[450px]:flex-col">
                                                        <div className="flex-col flex-1">
                                                            <p
                                                                id={isAdminView ? 'view_admin_oidc_clients_sub_title_id' : 'view_oidc_clients_sub_title_id'}
                                                                className="text-lg text-dark-blue mb-2"
                                                            >
                                                                {t('authenticationServices.oidcClientName')}: {oidcClientDetails.name}
                                                            </p>
                                                            <div className="flex items-center justify-start mb-2 max-[400px]:flex-col max-[400px]:items-start">
                                                                <div
                                                                    id={isAdminView ? 'view_admin_oidc_clients_status' : 'view_oidc_clients_status'}
                                                                    className={`${bgOfStatus(oidcClientDetails.status)} flex w-fit py-1 px-5 text-sm rounded-md my-2 font-semibold`}
                                                                >
                                                                    {getStatusCode(oidcClientDetails.status, t)}
                                                                </div>
                                                                <div
                                                                    id={isAdminView ? 'view_admin_oidc_clients_created_on' : 'view_oidc_clients_created_on'}
                                                                    className={`font-semibold ${isLoginLanguageRTL ? "mr-[1.4rem]" : "ml-[0.75rem]"} text-sm text-dark-blue`}
                                                                >
                                                                ...
                                                                <div
                                                                    id={isAdminView ? 'view_admin_oidc_clients_created_date_time' : 'view_oidc_clients_created_date_time'}
                                                        ...
                                                        <button
                                                            id="oidc_client_details_copy_id"
                                                            className={`${oidcClientDetails.status === "ACTIVE" ? 'bg-[#F0F5FF] border-[#BED3FF] cursor-pointer hover:shadow-md' : 'bg-gray-200 border-gray-400 cursor-default'} border h-[55px] w-[157px] max-[450px]:w-[40%] max-[800px]:w-[25%] px-3 py-2 rounded-md relative flex flex-col justify-center`}
                                                            onClick={() => copyClientId({ ...oidcClientDetails }, oidcClientDetails.id, setCopied)}
                                                            tabIndex={oidcClientDetails.status === "ACTIVE" ? "0" : "-1"}
                                                        >
                                                            <p
                                                                id={isAdminView ? 'view_admin_oidc_client_id_label' : 'view_oidc_client_id_label'}
                                                                className={`text-sm font-semibold text-[#333333] mb-1 ${isLoginLanguageRTL ? "text-right" : "text-left"}`}
                                                            >
                                                                {t('viewOidcClientDetails.oidcClientId')}
                                                            </p>
                                                            <div className={`flex space-x-1 items-center ${isLoginLanguageRTL ? "justify-start flex-row-reverse" : "justify-end"}`}>
                                                                <p
                                                                    id={isAdminView ? 'view_admin_oidc_client_id' : 'view_oidc_client_id'}
                                                                    className={`text-base font-bold ${oidcClientDetails.status === "ACTIVE" ? 'text-[#1447B2]' : 'text-gray-400'} truncate`}
                                                                >
                                                                ...
                                                                {copied && (
                                                                    <div
                                                                        ref={copyToolTipRef}
                                                                        className={`z-20 px-4 py-1 mt-10 min-h-8 font-semibold absolute ${isLoginLanguageRTL ? "left-8" : "right-8"} shadow-lg bg-white border border-gray-300 rounded-md`}
                                                                    >
                                                                        <p
                                                                            id={isAdminView ? 'view_admin_oidc_client_id_copied' : 'view_oidc_client_id_copied'}
                                                                            className="text-[#36393E] text-base font-inter"
                                                                        >
                                                                            {t('viewOidcClientDetails.copied!')}
                                                                        </p>
                                                                    </div>
                                                                )}
                                                            </div>
                                                        ...
                                                        <div className={`w-[49%] max-[600px]:w-[100%] mb-3 ${isLoginLanguageRTL ? "ml-[1%]" : "mr-[1%]"}`}>
                                                            <div className="mb-3">
                                                                ...
                                                                <p id='oidc_client_details_policy_group_name_context' className="font-[600] text-vulcan text-base">
                                                                    {isAdminView ? (oidcClientDetails.policyGroupName || ' - ') : oidcClientDetails.policyGroupName}
                                                                </p>
                                                            </div>
                                                            <div>
                                                                ...
                                                                <p id='oidc_client_details_policy_group_description_context' className="font-[600] text-vulcan text-base">
                                                                    {isAdminView ? (oidcClientDetails.policyGroupDescription || ' - ') : oidcClientDetails.policyGroupDescription}
                                                                </p>
                                                            </div>
                                                        </div>
                                                        {/* Policy Name Section */}
                                                        <div className={`w-[50%] max-[600px]:w-[100%] mb-3`}>
                                                            <div className="mb-3">
                                                                ...
                                                                <p id='oidc_client_details_policy_name_context' className="font-[600] text-vulcan text-base">
                                                                    {isAdminView ? (oidcClientDetails.policyName || ' - ') : oidcClientDetails.policyName}
                                                                </p>
                                                            </div>
                                                            <div>
                                                                ...
                                                                <p id='oidc_client_details_policy_description_context' className="font-[600] text-vulcan text-base">
                                                                    {isAdminView ? (oidcClientDetails.policyDescription || ' - ') : oidcClientDetails.policyDescription}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </div>
                                ...
                                <div className={`flex justify-end py-5`}>
                                    <button
                                        id="oidc_client_details_back_btn"
                                        onClick={moveToOidcClientsList}

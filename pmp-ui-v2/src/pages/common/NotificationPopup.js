                                        ...
                                        {notifications.map((notification, index) => (
                                            <div key={notification.notificationId} className={`flex justify-between items-start px-6 py-2 border-b border-gray-200 ${isLatestNotification(notification) ? 'bg-[#F0F6FF]' : ''}`}>
                                                ...
                                                <div className="mb-2">
                                                    ...
                                                    <div id={'notification_description_' + (index + 1)} className={`text-sm  ${isLatestNotification(notification) ? 'font-semibold' : 'font-normal'} text-[#344054] mt-1 mb-2 whitespace-pre-line break-words ${isLoginLanguageRTL ? 'text-right' : 'text-left'}`} dir={isLoginLanguageRTL ? 'rtl' : 'ltr'}>{getNotificationPanelDescription(notification, isLoginLanguageRTL, t)}</div>
                                                    <button

                ...
                <div onClick={openDropdown} className={`flex items-center ${isLoginLanguageRTL ? "mr-1" : "ml-1"}`} ref={dropdownRef}>
                    ...
                    {isDropdownOpen && (
                        <div className={`absolute top-14 ${isLoginLanguageRTL ? "left-1 origin-top-left" : "right-1 origin-top-right"} z-10 w-dynamic rounded-md bg-white py-1 shadow-md ring-1 ring-gray-50 focus:outline-none`}>
                            ...
                            <button className={`block w-full px-4 py-2 text-xs text-red-700 ${isLoginLanguageRTL ? "text-right" : "text-left"} hover:bg-gray-100`} onClick={logout}>{t('commons.logout')}</button>
                        </div>
                    )}
                </div>
            </div>

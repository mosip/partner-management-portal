    ...
    return (
        <nav className="sticky top-0 z-50 bg-white flex justify-between w-full h-14 font-inter 
                ...
                {!open && (
                    <div className={`flex items-center ${isLoginLanguageRTL ? "pr-5" : "pl-6"}`}>
                        ...
                        <div className="p-9 cursor-pointer" onClick={() => setOpen(!open)}>
                            <img className="min-w-5 w-5 h-5" src={hamburgerIcon} alt="" tabIndex="0" onKeyPress={(e) => {e.key==='Enter' && setOpen(!open)}}/>
                        </div>
                    </div>
                )}
                {open && (
                    <div className={`flex items-center w-64 gap-x-4 ${isLoginLanguageRTL ? "pr-5" : "pl-6"} h-14 shadow-md`}>
                        ...
                        <div className="cursor-pointer" onClick={() => setOpen(!open)}>
                            <img className="min-w-5" src={hamburgerIcon} alt="" tabIndex="0" onKeyPress={(e) => {e.key==='Enter' && setOpen(!open)}}/>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className={`flex items-center relative justify-between gap-x-12 ${isLoginLanguageRTL ?"left-3":"right-3"}`}>
                ...
                <div onClick={openDropdown} className={`flex items-center ${isLoginLanguageRTL ? "mr-1" : "ml-1"}`} ref={dropdownRef} tabIndex="0" onKeyPress={(e)=>{e.key==='Enter' && openDropdown()}}>
                    ...
                    {isDropdownOpen && (
                        <div className={`absolute top-[3.1rem] ${isLoginLanguageRTL ? "origin-top-left" : "origin-top-right"} z-10 w-dynamic rounded-md bg-white py-1 shadow-md ring-1 ring-gray-50 focus:outline-none`}>
                            <button onClick={() => moveToMyProfile()} className={`block w-full px-4 py-2 text-xs text-gray-900 ${isLoginLanguageRTL ? "text-right" : "text-left"} hover:bg-gray-100`} tabIndex="0" onKeyPress={(e) => {e.key==='Enter' && moveToMyProfile()}}>
                                {t('header.userProfile')}
                            </button>
                            <div className="border-gray-100 border-t mx-2"></div>
                            <button className={`block w-full px-4 py-2 text-xs text-red-700 ${isLoginLanguageRTL ? "text-right" : "text-left"} hover:bg-gray-100`} onClick={logout} onKeyPress={(e)=>{e.key==='Enter' && logout()}}>{t('commons.logout')}</button>
                        </div>
                    )}
                </div>
            </div>

        ...
        try {
            ...
            if (responseData && responseData.response) {
                ...
                if (responseData && responseData.errors && responseData.errors.length > 0) {
                    ...
                    if (popupData.isDeactivatePolicyGroup && (errorCode === 'PMS_POL_056' || errorCode === 'PMS_POL_069')) {
        ...
        try {
            ...
            if (responseData && responseData.response) {
                ...
                if (errorCode === 'PMS_POL_056') {
                    ...
                    count = resData.filter(policy => policy.is_Active && policy.schema).length;
                    setErrorHeaderMsg(t('activePoliciesDetectedMsg.header'));
                    setErrorDescriptionMsg(t('activePoliciesDetectedMsg.description', { noOfAssociatedPolicies: count }));
                } else if (errorCode === 'PMS_POL_069') {
                    // Count draft policies
                    count = resData.filter(policy => !policy.is_Active && !policy.schema).length;
                    setErrorHeaderMsg(t('draftPoliciesDetectedMsg.header'));
                    setErrorDescriptionMsg(t('draftPoliciesDetectedMsg.description', { noOfAssociatedPolicies: count }));
                }
            } else {

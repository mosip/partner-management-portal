...
function DeactivateApiKey({ closePopUp, apiKeyData }) {
    ...
    const clickOnConfirm = async () => {
        ...
        const request = createRequest({
            label: apiKeyData.apiKeyLabel,
            status: "De-Active"
        });
        try {
            const response = await HttpService.patch(getPartnerManagerUrl(`/partners/${apiKeyData.partnerId}/policy/${apiKeyData.policyId}/apiKey/status`, process.env.NODE_ENV), request, {
              headers: {
                    ...
                    <>
                        ...
                        <div className={`p-[10%] flex-col text-center justify-center items-center`}>
                            <p className="text-[17px] font-semibold text-black break-words px-[6%]">
                                {t('deactivateApiKey.apiKeyName')} - '{apiKeyData.apiKeyLabel}'?
                            </p>
                            <p className="text-sm text-[#666666] break-words py-[6%]">
                                {t('deactivateApiKey.description')}
                            </p>

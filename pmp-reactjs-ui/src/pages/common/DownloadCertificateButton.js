...
function DownloadCertificateButton({ setActiveDownloadBtn, activeDownloadBtn, firstOptionMethod, secondOptionMethod, partner, downloadDropdownRef, disableBtn, styleSet, index }) {
    ...
    return (
        <div ref={downloadDropdownRef} className={`flex-col`}>
            <button disabled={disableBtn} onClick={() => setActiveDownloadBtn(activeDownloadBtn === index ? null : index)}
                className={`h-10 ${isLoginLanguageRTL ? "ml-5" : "mr-5"} ${disableBtn ? 'bg-slate-200 text-slate-500 border-gray-200 border' : ''} flex items-center ${activeDownloadBtn === index ? 'bg-blue-800 text-white' : 'text-tory-blue bg-white'} text-xs px-[1rem] py-[1%] ${isLoginLanguageRTL ? "ml-1" : "mr-1"} text-tory-blue border border-blue-800 font-semibold rounded-lg text-center`}>
                {t('commons.download')}
                {!disableBtn ?
                    (activeDownloadBtn === index ?
            ...
            {activeDownloadBtn === index && (
                <div className={styleSet && styleSet.outerDiv}>
                    <div onClick={() => firstOptionMethod(partner)} className="flex items-center border-b justify-between cursor-pointer hover:bg-gray-100">
                    ...
                    <div onClick={() => secondOptionMethod(partner)} className="flex items-center justify-between cursor-pointer hover:bg-gray-100">

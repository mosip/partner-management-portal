...
function DropdownWithSearchComponent({ fieldName, dropdownDataList, onDropDownChangeEvent, fieldNameKey, 
    ...
    return (
        <div key={fieldName} className={`ml-4 mb-2 ${(styleSet && styleSet.outerDiv) ? styleSet.outerDiv : ''}`}>
            ...
            {showTooltip && (
                <div className={`z-20 p-4 -mt-[4.5%] w-[20%] max-h-[32%] overflow-y-auto absolute ${isLoginLanguageRTL?"mr-[7.8%]":"ml-[8.2%]"} ${selectedLang==='fra'?"right-[13%]":""} shadow-lg bg-white border border-gray-300 rounded`}>
                    <p className="text-black text-sm">{t(infoKey)}</p>
                </div>
            )}
            <div className="relative w-full" ref={dropdownRef}>
                <button onClick={openDropdown} disabled={disabled} className={`flex items-center justify-between w-fit h-10 px-2 py-2 border border-[#707070] bg-white rounded-[4px] text-[15px] ${selectedDropdownEntry ? 'text-[#343434]' : 'text-grayish-blue'} leading-tight focus:outline-none 
